...
function loadFns(env) {
  ...
  const runTween = () => {
    ...
    Object.assign(tw.obj, { x: tw.vars.x, y: tw.vars.y, zoom: tw.vars.zoom });
...
(async () => {
  ...
  // 2. sprawling content + lastEdit → fit zoom, centered on last edit
  env = makeEnv();
  env.strokes.set('a', rect('a', [0, 0, 2000, 1000]));
  f = loadFns(env);
  f.frameOnLoad({ x: 1900, y: 900 });
  check('fit: tween started', env._tweens.length === 1);
  const tw = env._tweens[0];
  check('fit: zoom fits content (0.2 min clamp)', approx(tw.vars.zoom, 0.2)); // min(400/2000,800/1000)*.92=.184→.2
  check('fit: centered on lastEdit', tw.vars.x === 1900 && tw.vars.y === 900);
  f.runTween();
  check('fit: tween paints via dirty', f.dirty === true);
  check('fit: cam reaches target', approx(f.cam.zoom, 0.2) && f.cam.x === 1900);

  // 3. no lastEdit → centered on content middle
  env = makeEnv();
  env.strokes.set('a', rect('a', [0, 0, 1000, 500]));
  f = loadFns(env);
  f.frameOnLoad(undefined);
  const tw3 = env._tweens[0];
  check('no-lastEdit: centered on bbox middle', tw3.vars.x === 500 && tw3.vars.y === 250);

  // 4. tiny content → never zoom in past 1x on open
  env = makeEnv();
  env.strokes.set('a', rect('a', [0, 0, 100, 100]));
  f = loadFns(env);
  f.frameOnLoad(null);
  check('tiny: zoom capped at 1x', approx(env._tweens[0].vars.zoom, 1));

  // 5. freehand bounds span all points; text uses textMetrics
  env = makeEnv({ w: 1000, h: 1000 });
  env.strokes.set('p', pen('p', [0, 0, 400, 0, 400, 300]));
  env.strokes.set('t', { id: 't', tool: 'text', x: 900, y: 900 });
  f = loadFns(env);
  f.frameOnLoad(null);
  const tw5 = env._tweens[0];
  ...
  check('mixed: zoom fits all', approx(tw5.vars.zoom, Math.min(1000 / 1005, 1000 / 925) * 0.92));
  check('mixed: centered on bbox middle', approx(tw5.vars.x, 497.5) && approx(tw5.vars.y, 457.5));
